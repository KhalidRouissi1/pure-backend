...
export default function AdminOrders() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      const matchesQuery =
        !q ||
        [order.id, order.user?.name, order.user?.email, order.paymentMethod, order.paymentStatus]
          .filter(Boolean)
    ...
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <View
          style={[
            styles.statusBadge,
            { backgroundColor: (statusColor[item.status] || colors.textTertiary) + '18' },
          ]}
        >
          <Text
            style={[styles.statusText, { color: statusColor[item.status] || colors.textTertiary }]}
          >
            {item.status}
          </Text>
        </View>
      ...
      <View style={styles.cardBody}>
        <View style={styles.cardRow}>
          ...
          <Text style={styles.cardMeta}>
            {item.user?.name || item.user?.email || t('admin:unknown')}
          </Text>
        </View>
        <View style={styles.cardRow}>
          <Ionicons name="cash-outline" size={14} color={colors.textSecondary} />
          <Text style={styles.cardMeta}>
            {t('common:sar')} {Number(item.total).toFixed(2)}
          </Text>
        </View>
        <View style={styles.cardRow}>
        ...
        <View style={styles.cardRow}>
          ...
          <Text style={styles.cardMeta}>
            {t('common:eta')} {item.deliveryEtaMinutes} {t('common:minutes')}
          </Text>
        </View>
      </View>
      {item.items?.length > 0 && (
          ...
          {item.items.map((it: any, i: number) => (
            <View key={i} style={styles.itemRow}>
              ...
              <Text style={styles.itemQty}>
                {it.quantity}x {t('common:sar')} {Number(it.unitPrice).toFixed(2)}
              </Text>
            </View>
          ))}
        </View>
      ...
      <View style={styles.cardFooter}>
        ...
        <Text style={styles.itemCount}>
          {t('admin:items_count', { count: item.items?.length || 0 })}
        </Text>
      </View>
    </View>
  );
          ...
          {[
            ...
            >
              <Text style={[styles.filterText, statusFilter === status && styles.filterTextActive]}>
                {status === 'ALL' ? t('common:all') : t(`admin:${status.toLowerCase()}`)}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  list: { paddingVertical: 16, paddingBottom: 40 },
