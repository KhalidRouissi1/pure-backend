...
export default function ProductDetail({ route, navigation }: any) {
  ...
  const mediaHeight = clamp(width * 0.65, 220, 420);
  ...
  const handleWhatsApp = () => {
    ...
    const message = encodeURIComponent(
      t('products:whatsappMessage', { name: productName, price })
    );
  ...
  const handleVisitStore = () => {
    if (product?.store?.id) {
      (navigation as any).navigate('StoreDetail', { storeId: product.store.id, store: product.store });
    }
  };
  ...
  const originAddress = product.originAddressText || store?.addressText || store?.city || product.originCity;
  const hasOriginMap = typeof originLatitude === 'number' && typeof originLongitude === 'number';

  const openOriginMaps = () => {
    if (!hasOriginMap) return;
    Linking.openURL(`https://www.google.com/maps/search/?api=1&query=${originLatitude},${originLongitude}`);
  };
      ...
      >
        {imageUrl ? (
          <View style={styles.imageContainer} testID="image-gallery">
            <Image source={{ uri: imageUrl }} style={[styles.image, { height: mediaHeight }]} />
        ...
        <View style={styles.content}>
          <View style={styles.priceRow}>
            <Text style={styles.price}>{product.price} {t('products:sar')}</Text>
            <View style={styles.actionButtons}>
              <TouchableOpacity style={styles.iconButton} onPress={handleShare} activeOpacity={0.7} testID="share-button">
                <Ionicons name="share-social-outline" size={20} color={colors.textSecondary} />
              </TouchableOpacity>
              <TouchableOpacity style={styles.iconButton} onPress={handleFavoriteToggle} activeOpacity={0.7} testID="favorite-button">
                <Ionicons
          ...
          {store && (
            <TouchableOpacity style={styles.storeCard} onPress={handleVisitStore} activeOpacity={0.7}>
            ...
            <View style={styles.originSection}>
              <View style={styles.originHeader}>
                <View>
                  ...
                  <Text style={styles.originText} numberOfLines={2}>{originAddress || product.originCity || t('common:saudi_arabia')}</Text>
                </View>
                {hasOriginMap ? (
          ...
          <View style={styles.purchasePanel}>
            <View>
              ...
              <View style={styles.stepper}>
                <TouchableOpacity style={styles.stepButton} onPress={() => setQuantity((value) => Math.max(1, value - 1))}>
                  <Ionicons name="remove" size={18} color={colors.text} />
                </TouchableOpacity>
                <Text style={styles.quantity}>{quantity}</Text>
                <TouchableOpacity style={styles.stepButton} onPress={() => setQuantity((value) => value + 1)}>
                  <Ionicons name="add" size={18} color={colors.text} />
                </TouchableOpacity>
              </View>
            </View>
            <TouchableOpacity style={styles.cartButton} onPress={handleAddToCart} disabled={adding}>
              <Ionicons name="basket-outline" size={20} color="#FFFFFF" />
              <Text style={styles.cartButtonText}>{adding ? t('products:adding') : t('products:add_to_cart')}</Text>
            </TouchableOpacity>
          </View>

          <TouchableOpacity style={styles.reviewsButton} onPress={() => navigation.navigate('Reviews', { productId })}>
            <View>
              <Text style={styles.sectionTitle}>{t('products:reviews')}</Text>
              <Text style={styles.reviewSummary}>{t('products:review_summary', { rating: product.averageRating || 0, count: product.reviewCount || 0 })}</Text>
            </View>
            <Ionicons name="chevron-forward" size={18} color={colors.textTertiary} />
          </TouchableOpacity>

          <TouchableOpacity style={styles.whatsappButton} onPress={handleWhatsApp} activeOpacity={0.7} testID="whatsapp-button">
            <LinearGradient
...
const styles = StyleSheet.create({
  ...
  emptyText: { fontFamily: FontFamilies.regular, fontSize: 15, color: colors.textTertiary, marginTop: 12 },
  ...
  imagePlaceholderText: { fontFamily: FontFamilies.regular, fontSize: 14, color: colors.textTertiary, marginTop: 8 },
  content: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 40 },
  priceRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 6 },
  price: { flex: 1, fontFamily: FontFamilies.bold, fontSize: 30, color: colors.primaryDark },
  ...
  productName: { fontFamily: FontFamilies.bold, fontSize: 22, color: colors.text, marginBottom: 12, lineHeight: 32 },
  categoryBadge: {
  ...
  storeMetaRow: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginTop: 2 },
  ...
  originSection: { backgroundColor: colors.surface, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: colors.borderLight, marginBottom: 12 },
  originHeader: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 },
  originText: { fontFamily: FontFamilies.regular, color: colors.textSecondary, lineHeight: 20 },
  directionsButton: { flexDirection: 'row', alignItems: 'center', gap: 4, borderRadius: 12, borderWidth: 1, borderColor: '#BFE7BF', paddingHorizontal: 10, paddingVertical: 7 },
  ...
  purchasePanel: { backgroundColor: colors.surface, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: colors.borderLight, marginBottom: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' },
  stepper: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 8 },
  stepButton: { width: 34, height: 34, borderRadius: 17, backgroundColor: colors.secondarySoft, justifyContent: 'center', alignItems: 'center' },
  quantity: { minWidth: 24, textAlign: 'center', fontFamily: FontFamilies.bold, fontSize: 16, color: colors.text },
  cartButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#0AAD0A', borderRadius: 16, paddingHorizontal: 18, paddingVertical: 14, minHeight: 48, flexGrow: 1 },
  cartButtonText: { fontFamily: FontFamilies.bold, color: '#FFFFFF' },
  reviewsButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.surface, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: colors.borderLight, marginBottom: 12 },
