...
export default function StoreDetail({ route, navigation }: any) {
  ...
  const load = useCallback(async () => {
    if (!storeId) return;
    try {
      const [storeResult, productsResult, reviewsResult] = await Promise.allSettled([
        getStore(storeId),
      ...
      if (reviewsResult.status === 'fulfilled') setReviewSummary(reviewsResult.value.summary);
    } finally {
      setLoading(false);
    }
  }, [storeId]);
  ...
  return (
    <View style={styles.root}>
      ...
      >
        <ResponsiveContent padded={false} style={[styles.hero, layout.isTablet && styles.heroWide]}>
          {heroImage ? (
        ...
        <ResponsiveContent
          style={[styles.statsWrapper, layout.isTablet && styles.statsWrapperWide]}
        >
          <View style={styles.statsRow}>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('Reviews', { storeId })}
            >
              <Text style={styles.statValue}>
                {reviewSummary.averageRating || store.averageRating || 0}
              </Text>
              <Text style={styles.statLabel}>{t('stores:rating')}</Text>
            </TouchableOpacity>
            <View style={styles.statDivider} />
            <View style={styles.statItem}>
              <Text style={styles.statValue}>{store.category}</Text>
              <Text style={styles.statLabel}>{t('common:category')}</Text>
            </View>
          </View>
        </ResponsiveContent>
        ...
        <ResponsiveContent style={styles.content}>
          ...
          {store.certificationStatus && (
            <Text style={styles.certification}>
              {t('stores:certification_status', { status: store.certificationStatus })}
            </Text>
          )}
        </ResponsiveContent>

        <ResponsiveContent style={styles.productsSection}>
          ...
          {products.length > 0 ? (
            ...
            <View style={styles.emptyProducts}>
              <Ionicons name="basket-outline" size={34} color={colors.textTertiary} />
              <Text style={styles.emptyProductsText}>{t('products:no_products_store')}</Text>
            </View>
          )}
        </ResponsiveContent>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  heroWide: { height: 340, borderBottomLeftRadius: 24, borderBottomRightRadius: 24 },
  ...
  statsWrapper: { marginTop: -22 },
  statsWrapperWide: { maxWidth: 760 },
  statsRow: {
    flexDirection: 'row',
  ...
  productsSection: { marginTop: 6, marginBottom: 20 },
  productList: { gap: 12 },
  emptyProducts: {
    minHeight: 120,
    marginTop: 12,
