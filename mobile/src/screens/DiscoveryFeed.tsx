...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, ActivityIndicator, Platform, TextInput, useWindowDimensions } from 'react-native';
...
export default function DiscoveryFeed({ navigation }: any) {
  ...
  const bannerWidth = clamp(width - 48, 248, 320);
  ...
  const loadDiscoveryData = async (showLoading = false) => {
    ...
    try {
      try { const d = await getDiscoveryFeed({ groupBy, limit: 20 }); setGroups(d?.data?.groups || []); } catch {}
      try { const d = await getTrendingProducts({ period: '7d', limit: 20 }); setTrending(d?.data?.trending || []); } catch {}
      try { const d = await getNewProducts({ days: 7, limit: 10 }); setNewProducts(d?.data?.newProducts || []); } catch {}
      try { const stores = await getStores({ verified: true, limit: 8 }); setNearbyFarms(stores); } catch {}
  ...
  const visibleGroups = useMemo(() => groups.filter((group) => group.products.length > 0), [groups]);

  const handleProductPress = (product: Product) => { navigation.navigate('ProductDetail', { productId: product.id, product }); };
  const handleFavoriteToggle = async (productId: string, isFavorited: boolean) => {
    try { 
      const { toggleFavorite } = require('../services/products'); 
  ...
  const renderGroup = ({ item }: { item: any }) => (
    <View style={styles.groupSection}>
      <View style={styles.sectionHeader}>
        <View><Text style={styles.sectionTitle}>{item.groupLabel}</Text><View style={styles.sectionAccent} /></View>
        <TouchableOpacity style={styles.seeAllButton}><Text style={styles.seeAllText}>{t('discovery:seeAll')}</Text><Ionicons name="chevron-forward" size={14} color={colors.primary} /></TouchableOpacity>
      </View>
      <FlatList data={item.products} renderItem={renderRailProductCard} keyExtractor={(p) => p.id} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalList} />
    </View>
  );
    ...
    <View style={styles.root}>
      ...
      >
        ...
        >
          {categoryKeys.map((cat) => {
            const config: Record<Category, { icon: string; color: string; iconColor: string }> = {
              [Category.FRUITS_VEGETABLES]: { icon: 'leaf', color: '#E1F8E1', iconColor: '#0AAD0A' },
              ...
              [Category.NATURAL_BEAUTY]: { icon: 'flower', color: '#F1E1F8', iconColor: '#800080' },
            };
            ...
            return (
              ...
              >
                <View style={[styles.iconBox, { backgroundColor: item.color }, selectedCategory === cat.name && { borderWidth: 2, borderColor: item.iconColor }]}>
                  <Ionicons name={item.icon as any} size={28} color={item.iconColor} />
                </View>
                <Text style={[styles.categoryIconLabel, selectedCategory === cat.name && { color: item.iconColor, fontWeight: 'bold' }]}>
                  {t(cat.labelKey)}
                </Text>
              </TouchableOpacity>
            );
        ...
        <View style={styles.groupToggle}>
          ...
          <TouchableOpacity
            testID="region-tab"
            style={[styles.groupToggleButton, groupBy === 'region' && styles.groupToggleButtonActive]}
            onPress={() => setGroupBy('region')}
          >
            <Text style={[styles.groupToggleText, groupBy === 'region' && styles.groupToggleTextActive]}>
              {t('common:region_tab')}
            </Text>
          </TouchableOpacity>
        </View>
          ...
          <>
            ...
            <View style={styles.promoCard}>
              <View style={styles.promoContent}>
                <Text style={styles.promoPrice}>{t('common:sar')} 10</Text>
              ...
              <View style={styles.storesSection}>
                <View style={styles.storesGrid}>
                  {nearbyFarms.slice(0, 7).map((item) => (
                    <TouchableOpacity 
                      key={item.id} 
                      style={styles.storeMiniCard} 
                      onPress={() => navigation.navigate('StoreDetail', { storeId: item.id, store: item })}
                    >
                      <View style={styles.storeLogoBox}>
                        <Ionicons name="storefront" size={24} color={colors.primary} />
                      </View>
                        <Text style={styles.storeMiniName} numberOfLines={1}>{item.name}</Text>
                        <Text style={styles.storeMiniMeta} numberOfLines={1}>{item.city || ''}</Text>
                    </TouchableOpacity>
                  ))}
                  <TouchableOpacity style={styles.storeMiniCard} onPress={() => navigation.navigate('ProductList', { mode: 'stores' })}>
                    <View style={[styles.storeLogoBox, { backgroundColor: '#F0F0F0' }]}> 
                      <Ionicons name="arrow-forward" size={24} color="#666" />
                    </View>
                    <Text style={styles.storeMiniName}>{t('discovery:seeAll')}</Text>
                    <Text style={styles.storeMiniMeta}>{nearbyFarms.length} {t('common:store')}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            )}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bannersScroll}>
              <View style={[styles.bannerCard, { width: bannerWidth, backgroundColor: '#153A20' }]}>
              ...
              <View style={[styles.bannerCard, { width: bannerWidth, backgroundColor: colors.primary }]}>
                <View style={styles.bannerText}>
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <View><Text style={styles.sectionTitle}>{t('discovery:trending')}</Text></View>
                <TouchableOpacity style={styles.seeAllButton}><Text style={styles.seeAllText}>{t('discovery:seeAll')}</Text></TouchableOpacity>
              </View>
              {visibleTrending.length > 0 ? (
                <FlatList data={visibleTrending} renderItem={renderRailProductCard} keyExtractor={(item) => item.id} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalList} />
              ) : (
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <View><Text style={styles.sectionTitle}>{t('discovery:newArrivals')}</Text></View>
                <TouchableOpacity style={styles.seeAllButton}><Text style={styles.seeAllText}>{t('discovery:seeAll')}</Text></TouchableOpacity>
              </View>
              {visibleNewProducts.length > 0 ? (
                <FlatList data={visibleNewProducts} renderItem={renderRailProductCard} keyExtractor={(item) => item.id} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalList} />
              ) : (
                <Text style={styles.emptySectionText}>{t('discovery:noProducts')}</Text>
              )}
            </View>

            <FlatList data={visibleGroups} renderItem={renderGroup} keyExtractor={(item) => item.groupName} scrollEnabled={false} />
          </>
        )}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  searchBar: { height: 50, borderRadius: 25, backgroundColor: '#F2F2F2', flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 18 },
  ...
  iconBox: { width: 64, height: 64, borderRadius: 18, justifyContent: 'center', alignItems: 'center', position: 'relative' },
  categoryIconLabel: { fontFamily: FontFamilies.semiBold, fontSize: 13, color: colors.text },
  badge: { position: 'absolute', top: -5, right: -5, backgroundColor: '#FF4500', borderRadius: 10, paddingHorizontal: 6, paddingVertical: 2 },
  badgeText: { color: '#FFFFFF', fontSize: 10, fontWeight: 'bold' },
  groupToggle: { flexDirection: 'row', marginHorizontal: 16, marginBottom: 16, padding: 4, borderRadius: 18, backgroundColor: '#F2F2F2' },
  groupToggleButton: { flex: 1, minHeight: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 12 },
  groupToggleButtonActive: { backgroundColor: '#FFFFFF' },
  ...
  promoCard: { marginHorizontal: 16, marginBottom: 20, padding: 18, borderRadius: 20, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F0F0F0', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 10 },
  promoContent: { flex: 1, paddingRight: 10 },
  promoPrice: { fontSize: 24, fontWeight: 'bold', color: colors.text },
  promoTitle: { fontSize: 16, fontWeight: 'bold', color: colors.text, marginTop: 4 },
  promoSub: { fontSize: 13, color: '#666', marginTop: 4 },
  promoButton: { backgroundColor: '#F0F0F0', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, minHeight: 44, justifyContent: 'center' },
  ...
  storeMiniCard: { width: '22%', minWidth: 70, flexGrow: 0, flexShrink: 0, alignItems: 'center', gap: 6 },
  storeLogoBox: { width: '100%', aspectRatio: 1, borderRadius: 15, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#F0F0F0', justifyContent: 'center', alignItems: 'center' },
  ...
  bannerBtn: { alignSelf: 'flex-start', backgroundColor: '#FFFFFF', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 18 },
  ...
  searchResultsTitle: { paddingHorizontal: 16, fontFamily: FontFamilies.bold, fontSize: 18, color: colors.text, marginBottom: 16 },
  searchGrid: { paddingHorizontal: 16, paddingBottom: 20 },
  columnWrapper: { gap: 12 },
  emptySearch: { padding: 40, alignItems: 'center', justifyContent: 'center' },
  emptySearchTitle: { marginTop: 12, fontFamily: FontFamilies.semiBold, fontSize: 16, color: colors.textSecondary },
});
