...
export default function Checkout({ navigation }: any) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'CASH_ON_DELIVERY' | 'CARD_ON_DELIVERY'>(
    'CASH_ON_DELIVERY'
  );
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSelectedAddressId(
        addressData.find((address) => address.isDefault)?.id || addressData[0]?.id
      );
    } finally {
      setLoading(false);
    }
  ...
  const paymentMethods = [
    {
      ...
      label: t('common:cash_on_delivery'),
      icon: 'cash-outline' as const,
    },
    {
      id: 'CARD_ON_DELIVERY' as const,
      label: t('common:card_on_delivery'),
  ...
  const renderPaymentIcon = (method: (typeof paymentMethods)[number]) => (
    <Ionicons
      name={method.icon}
      size={20}
      color={paymentMethod === method.id ? '#0AAD0A' : colors.textSecondary}
    />
  );
      ...
      >
        <View style={styles.panel}>
          ...
          {addresses.length ? (
            addresses.map((address) => (
              <TouchableOpacity
                key={address.id}
                style={[styles.option, selectedAddressId === address.id && styles.optionSelected]}
          ...
          <View style={styles.deliveryMeta}>
            <Text style={styles.muted}>
              {t('common:delivery_fee')}: {t('common:sar')} {deliveryFee.toFixed(2)}
            </Text>
            <Text style={styles.muted}>
              {t('common:eta')}: 45 {t('common:minutes')}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.panel}>
          ...
          {paymentMethods.map((method) => (
            <TouchableOpacity
              key={method.id}
              style={[styles.option, paymentMethod === method.id && styles.optionSelected]}
