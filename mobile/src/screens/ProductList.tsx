...
export default function ProductList({ route, navigation }: any) {
  ...
  const storeColumns = grid.isWide ? 2 : 1;
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) =>
      [product.name, product.description, product.category, product.store?.name]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(q))
    );
  }, [query, products]);
  ...
  const subtitle =
    mode === 'stores'
      ...
      : storeId
        ? t('products:products_from_store')
      ...
      >
        ...
        <View style={styles.storeInfo}>
          ...
          <Text style={styles.storeMeta} numberOfLines={1}>
            {item.city || t('common:other')} · {item.category}
          </Text>
  ...
  return (
    <View style={styles.root}>
      <BackHeader title={title} subtitle={subtitle} />
      <View style={[styles.searchWrap, { marginHorizontal: grid.listHorizontalPadding }]}>
      ...
      {mode === 'stores' ? (
        <FlatList
          key={`stores-${storeColumns}`}
          data={filteredStores}
          keyExtractor={(item) => item.id}
          renderItem={renderStore}
          numColumns={storeColumns}
          columnWrapperStyle={storeColumns > 1 ? styles.columnWrapper : undefined}
          contentContainerStyle={[
            styles.listContent,
            { paddingBottom: insets.bottom + 24, paddingHorizontal: grid.listHorizontalPadding },
          ]}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}
          ListEmptyComponent={
            <EmptyState
              title={t('stores:no_stores_found')}
              subtitle={t('stores:try_different_filter')}
            />
          }
        />
      ) : (
        <FlatList
          key={`products-${grid.columns}`}
          ...
          ListEmptyComponent={
            <EmptyState
              title={t('common:no_products')}
              subtitle={t('products:no_products_store')}
            />
          }
        />
      )}
