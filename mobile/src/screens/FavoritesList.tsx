...
export default function FavoritesList({ navigation }: any) {
  ...
  const loadFavorites = useCallback(async () => {
    try {
      ...
      setFavorites(r as FavoriteItem[]);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
    ...
    <View style={styles.root}>
        ...
        {favorites.length > 0 && (
          <View style={styles.countBadge}>
            <Text style={styles.countText}>
              {favorites.length} {t('profile:saved_count')}
            </Text>
          </View>
        )}
      </View>
      <FlatList
        ...
        contentContainerStyle={[
          ...
          {
            paddingHorizontal: grid.listHorizontalPadding,
            maxWidth: grid.maxContentWidth + grid.outerInset * 2,
          },
        ]}
