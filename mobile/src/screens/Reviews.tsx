...
import { createProductReview, createStoreReview, getProductReviews, getStoreReviews } from '../services/reviews';
      ...
      <FlatList
            ...
            <View style={styles.stars}>
              {[1, 2, 3, 4, 5].map((value) => (
                <TouchableOpacity key={value} onPress={() => setRating(value)}>
                  <Ionicons name={value <= rating ? 'star' : 'star-outline'} size={24} color="#F7A600" />
                </TouchableOpacity>
              ))}
            </View>
            <TextInput label={t('write_review')} value={comment} onChangeText={setComment} mode="outlined" multiline />
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{'★'.repeat(item.rating)}{'☆'.repeat(5 - item.rating)}</Text>
...
const styles = StyleSheet.create({
  ...
  primaryButton: { backgroundColor: '#0AAD0A', borderRadius: 16, paddingVertical: 14, alignItems: 'center', marginTop: 12 },
  ...
  emptyText: { fontFamily: FontFamilies.regular, color: colors.textSecondary, textAlign: 'center', marginTop: 20 },
});
