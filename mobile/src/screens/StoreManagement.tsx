...
export default function StoreManagement({ navigation }: any) {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'verified' | 'unverified' | 'trusted'>(
    'all'
  );
  ...
  const handleDeleteStore = (storeId: string, storeName: string) => {
    Alert.alert(t('stores:delete_store'), t('stores:delete_store_confirm', { name: storeName }), [
  ...
  const filteredStores = useMemo(() => {
    ...
    return stores.filter((store) => {
      ...
      const matchesQuery =
        !q ||
        [store.name, store.city, store.category, ownerEmail]
          .filter(Boolean)
  ...
  const renderStoreItem = ({ item }: { item: Store }) => (
    <TouchableOpacity
      ...
      accessibilityLabel={`${item.name} ${t('products:products_from_store')}`}
    >
      <View style={styles.storeHeader}>
        ...
        <View style={styles.storeDetails}>
          ...
          <Text style={styles.ownerEmail} numberOfLines={1}>
            {(item as any).owner?.email}
          </Text>
          <View style={styles.statusRow}>
            <View
              style={[styles.badge, item.isVerified ? styles.verifiedBadge : styles.pendingBadge]}
            >
              <Text
                style={[
                  styles.badgeText,
        ...
        <TouchableOpacity
          ...
          onPress={(event) => {
            ...
            handleDeleteStore(item.id, item.name);
          }}
        >
  ...
  return (
    <View style={styles.root}>
      <BackHeader
        title={t('stores:store_management')}
        subtitle={t('stores:store_management_subtitle')}
      />
